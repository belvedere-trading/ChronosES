...
class SubscriptionManagerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.mockEventStore.KeyGenerator.return_value.hashKey = 'Chronos.TestName'
        self.mockEventStore.KeyGenerator.return_value.GetFailureTopic.return_value = 'Failure.Chronos.TestName'
        self.mockEventStore.KeyGenerator.return_value.GetManagementTopic.return_value = 'Management.Chronos.TestName'
        ...
        self.subscriptionManager = SubscriptionManager(self.mockEventStore, DummyAggregate,
    ...
    def test_SubscribeForUnifiedUpdates_should_subscribe_despite_errors(self, name, sideEffects):
        self.mockEventStore.Subscribe.side_effect = sideEffects
        self.subscriptionManager.SubscribeForUnifiedUpdates()

        self.assertEqual(self.mockEventStore.Subscribe.call_args_list,
    ...
    def test_UnsubscribeForUnifiedUpdates_should_unsubscribe_despite_errors(self, name, sideEffects):
        self.mockEventStore.Unsubscribe.side_effect = sideEffects
        self.subscriptionManager.UnsubscribeForUnifiedUpdates()

        self.assertEqual(self.mockEventStore.Unsubscribe.call_args_list,
    ...
    def test_Subscribe_with_no_indices_should_subscribe_wildcard_channel(self):
        ...
        self.mockEventStore.Subscribe.assert_called_once_with('Chronos.TestName', 'Chronos.TestName.*.*', self.mockSuccessCallback)
    ...
    def test_Subscribe_with_some_indices_should_subscribe_partial_wildcard_channel(self):
        ...
        self.mockEventStore.Subscribe.assert_called_once_with('Chronos.TestName', 'Chronos.TestName.1.*.3', self.mockSuccessCallback)
    ...
    def test_Subscribe_with_all_indices_should_subscribe_concrete_channel(self):
        ...
        self.mockEventStore.Subscribe.assert_called_once_with('Chronos.TestName', 'Chronos.TestName.baz.quux', self.mockSuccessCallback)
    ...
    def test_Unsubscribe_without_prior_Subscribe_should_not_fail(self):
        ...
        self.assertEqual(self.mockEventStore.Unsubscribe.call_count, 0)
    ...
    def test_Unsubscribe_should_unsubscribe(self):
        ...
        self.mockEventStore.Unsubscribe.assert_called_once_with('Chronos.TestName', 'Chronos.TestName')
    ...
    def test_UnsubscribeAll_should_unsubscribe_all_subscribed_channels(self):
        ...
        self.mockEventStore.Unsubscribe.side_effect = [None, ValueError('boom'), IndexError('out of range')]
        ...
        self.assertEqual(self.mockEventStore.Unsubscribe.call_count, 3)
        callList = self.mockEventStore.Unsubscribe.call_args_list
    ...
    def test_Resubscribe_without_existing_subscriptions_should_only_set_indexedAttributes(self):
        ...
        self.assertEqual(self.mockEventStore.Subscribe.call_count, 0)
        self.assertEqual(self.mockEventStore.Unsubscribe.call_count, 0)
    ...
    def test_Resubscribe_with_same_indexedAttributes_should_do_nothing(self):
        ...
        self.assertEqual(self.mockEventStore.Unsubscribe.call_count, 0)
        self.assertEqual(self.mockEventStore.Subscribe.call_count, 1)
    ...
    def test_Resubscribe_with_new_indexedAttributes_should_migrate_all_subscriptions(self):
        ...
        self.mockEventStore.Unsubscribe.side_effect = [ValueError('something'), None]
        self.mockEventStore.Subscribe.side_effect = [None, KeyError('something else')]
        self.subscriptionManager.Resubscribe(['attribute', 'newAttribute', 'second'])

        self.assertEqual(self.mockEventStore.Unsubscribe.call_count, 2)
        self.assertTrue(mock.call('Chronos.TestName', 'Chronos.TestName.foo.*') in self.mockEventStore.Unsubscribe.call_args_list)
        self.assertTrue(mock.call('Chronos.TestName', 'Chronos.TestName.*.2') in self.mockEventStore.Unsubscribe.call_args_list)

        self.assertEqual(self.mockEventStore.Subscribe.call_count, 4)
        self.assertTrue(mock.call('Chronos.TestName', 'Chronos.TestName.foo.*.*', self.mockSuccessCallback) in self.mockEventStore.Subscribe.call_args_list)
        self.assertTrue(mock.call('Chronos.TestName', 'Chronos.TestName.*.*.2', self.mockSuccessCallback) in self.mockEventStore.Subscribe.call_args_list)
